...
`bin/supervise-codex.sh` guarantees one supervisor, avoids racing an already-live runner, retries clean stops on the same thread, and exponentially backs off after short or failed runs. Each evaluation runs behind `bin/milestone-gate-watchdog.sh`, a distinct child subreaper that retains the control/runner leases independently of the supervisor and evaluator. If either inner owner dies, active checks and descriptor-closing detached children reparent to the watchdog, which retries cleanup without releasing the leases.

Milestone evaluation holds the control and runner kernel leases across the complete active process chain: supervisor, gate watchdog, evaluator, and current evidence check. The evaluator rejects a check that returns with any executable descendant, including redirected `setsid` or double-forked work outside the original process group. The watchdog independently verifies its adopted tree after evaluation. A killed cleanup verifier is retried while both leases remain held and leaves a persistent quarantine marker. `GATE-ACTIVE` is removed only after verified cleanup; runner, restart, and supervisor entry points reject a stale marker even if the kernel locks are free. Before an incomplete gate can launch Codex, the supervisor closes evaluator-era descriptions and acquires both locks afresh.
