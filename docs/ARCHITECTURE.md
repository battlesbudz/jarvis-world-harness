...
`bin/supervise-codex.sh` is the outer watchdog and the persistent child-subreaper around gate evaluation. It guarantees one supervisor, avoids racing an already-live runner, retries clean stops on the same thread, and exponentially backs off after short or failed runs. If the evaluator is hard-killed, active checks and detached children that closed every inherited descriptor reparent to the still-live supervisor; it terminates that complete tree before releasing the control/runner leases.

Milestone evaluation holds the control and runner kernel leases across the complete active process chain: supervisor, gate evaluator, and current evidence check. The evaluator rejects a check that returns with any executable descendant, including redirected `setsid` or double-forked work outside the original process group. The supervisor independently verifies and cleans its adopted tree after evaluation, and treats any fallback cleanup as an infrastructure failure rather than valid evidence. Before an incomplete gate can launch Codex, it closes evaluator-era descriptions and acquires both locks afresh.
